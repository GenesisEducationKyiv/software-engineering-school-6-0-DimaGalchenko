...
describe("ScannerService", () => {
  ...
  describe("scan", () => {
    it("fetches releases for each confirmed repo", async () => {
      ...
      expect(deps.githubService.fetchReleases).toHaveBeenCalledWith(
        "owner/repo1",
      );
      expect(deps.githubService.fetchReleases).toHaveBeenCalledWith(
        "owner/repo2",
    ...
    it("skips repos with no releases", async () => {
      ...
      expect(
        deps.subscriptionRepository.findConfirmedByRepo,
    ...
    it("skips scan if previous scan is still running", async () => {
      ...
      expect(
        deps.subscriptionRepository.findDistinctConfirmedRepos,
